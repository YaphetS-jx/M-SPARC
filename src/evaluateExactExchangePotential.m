    ...
    for i = 1:size(X,2)
        for ind = 1:S.Nd_subd
            ...
            for q_ind = 1:S.tnkpthf
                ...
                dsn = density_matrix_row(S.psi_outer(:,:,q_ind_rd+spin_shift),S.occ_outer(:,q_ind_rd+spin_shift),nd);
                rhs = dsn .* X(:,i);
                q = S.kptgridhf(q_ind,:);
                k_shift = kptvec - q;
                v = poissonSolve_FFT(S,rhs,k_shift);
                Vexx(nd,i) = Vexx(nd,i) - S.wkpthf(q_ind)*v(nd);
            end
        end
    end
    return;
...
% Compute col-th row of density matrix
