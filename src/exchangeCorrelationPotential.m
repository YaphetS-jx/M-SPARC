...
function [sxsr,v1xsr,v2xsr] = pbexsr(rho_in,grho_in,omega)
% constants 
...
sxsr = zeros(size(rho_in));
v1xsr = zeros(size(rho_in));
v2xsr = zeros(size(rho_in));

for ir = 1:length(rho_in)
    rho = rho_in(ir);
    grho = grho_in(ir);
    ...
    sxsr(ir)  = ex*fx;
    dsdn  = -4/3*s/rho;
    v1xsr(ir) = vx*fx + (dsdn*d2x+d1x)*ex;
    dsdg  = us*rr;
    v2xsr(ir) = ex/sqrt(aa)*dsdg*d2x;
end
